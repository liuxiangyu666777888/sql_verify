...
路由     & Vue Router 4.3 & 15 条路由，角色级导航守卫 \\
HTTP     & Axios 1.6 & JWT 拦截器，401 自动跳转登录 \\
样式     & Tailwind CSS 3.4 & 自定义设计令牌（颜色、字体、间距） \\
SQL 输入 & \texttt{<textarea>} & 原生文本输入，草稿 localStorage 自动保存 \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \texttt{router/index.ts}：15 条路由，按角色分三级——公开路由（/login）、学生路由（/student/*）、教师路由（/teacher/*）、管理员路由（/admin/*）。路由守卫检查 Pinia auth store 的角色字段，学生访问教师路由自动重定向到学生仪表盘，反之同理。未登录访问任何受保护路由都重定向 /login。
\end{itemize}
