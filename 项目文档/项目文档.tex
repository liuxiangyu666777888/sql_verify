...
\begin{description}[leftmargin=2em]
  ...
  \item[助教（ASSISTANT）] 复用教师端工作台和教师权限，用于协助教师维护题库、班级、考试和成绩统计；系统中通过角色名称区分助教身份。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{题库管理}：教师和助教创建/编辑/删除题目，支持 Markdown 题面、表结构展示、难度分级、标签分类。学生浏览和搜索题目。
  \item \textbf{SQL 判题}：学生在浏览器端编写 SQL，支持运行自测和正式提交。系统自动执行判题，返回 AC/WA/ERROR/TLE/FORBIDDEN 状态。
  \item \textbf{在线考试}：教师和助教通过四步向导创建考试（配置参数 → 选题 → 设分值 → 分配学生），发布后学生在指定时间窗口内作答，后端限制开始前和结束后的考试提交。作答页支持多题导航，每道题草稿保存在浏览器本地，刷新后可恢复。
  \item \textbf{班级管理}：教师和助教创建班级并生成邀请码，学生通过邀请码加入班级。
  \item \textbf{成绩统计}：教师和助教查看考试成绩排名、平均分、最高分。成绩由服务端自动汇总每题最高分，不可前端篡改。
  \item \textbf{用户管理}：管理员创建/编辑用户，调整角色和账号状态。
  \item \textbf{仪表盘}：学生端展示已解决题目数、正确率、连续练习天数、即将考试、可用题目列表。教师端展示班级数、题库量、待处理提交、最近考试；助教登录后进入同一教师端页面。
\end{enumerate}
...
系统包含 9 张核心数据表，实体关系如图 \ref{fig:er} 所示。

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{ER图.png}
\caption{数据库 E-R 图}
\label{fig:er}
\end{figure}
...
\begin{enumerate}[leftmargin=2em]
  \item \texttt{questions.description} 使用 Markdown 格式存储题面，前端以纯文本展示。
...
\begin{itemize}
  ...
  \item \textbf{禁止}：INSERT、UPDATE、DELETE、DROP、ALTER、CREATE、TRUNCATE、GRANT、REVOKE、LOAD DATA，以及任何形式的多语句执行。
\end{itemize}
...
通过 \texttt{indexOf(`;') != lastIndexOf(`;')} 比较首个分号和末个分号的字符串位置来判断：如果两者不同说明存在至少两个分号即多语句，拒绝执行。此方法无法区分字符串常量内的分号，但单个分号（含在字符串内的）不会被误拦。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \begin{enumerate}
    ...
    \item 执行学生 SQL（设置 \texttt{setQueryTimeout} = 5 秒），通过独立连接和临时 Schema 实现安全隔离。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item 创建 Schema 和初始化表结构使用管理员连接（需 \texttt{allowMultiQueries}），执行学生 SQL 使用独立连接并关闭多语句；部署者可通过 \texttt{JUDGE\_DB\_*} 配置专用低权限账号。
  \item SQL 提交前做语法级检查：关键字黑名单 → 分号计数 → JSqlParser AST 解析。
  \item 单次查询结果行数受配置约束（\texttt{max-rows}），防止超大结果集拖垮服务。
  \item 判题失败也必须清理临时 Schema，清理逻辑放在 \texttt{finally} 块中保证执行。
  \item 系统启动时自动扫描并清理残留的孤儿 \texttt{judge\_\%} Schema，防止 JVM 异常退出导致临时库堆积。
  \item 测试用例的初始化 SQL 只能由教师或助教创建，学生不可编辑。
\end{enumerate}
...
统一响应封装（\texttt{ApiResponse}）、自定义业务异常（\texttt{BusinessException}，错误码 40000--40400）、全局异常处理（\texttt{GlobalExceptionHandler}）、当前用户工具类（\texttt{CurrentUser}）、分页响应（\texttt{PageResponse}）。
...
题库完整 CRUD。教师、助教和管理员可创建/编辑/软删除题目（\texttt{visible = 0}）。搜索支持关键词同时匹配标题和描述，服务端分页。题目展示难度标签，但未实现按难度筛选。测试用例管理：增删改查，\texttt{expected\_output} 以 JSON 格式存储。权限控制：学生只看到 \texttt{visible = 1} 的题目，答案和测试用例字段在学生返回中为 null。

\item[judge 模块（8 文件 / 453 行）]
系统核心。\texttt{JudgeService}：三层安全检查 → 临时 Schema 创建 → 测试用例初始化 → 学生 SQL 执行 → 结果比对 → Schema 清理。管理员连接负责建库（\texttt{allowMultiQueries=true}），学生 SQL 走独立连接并关闭多语句（\texttt{allowMultiQueries=false}）。启动时自动清理残留的孤儿 \texttt{judge\_\%} Schema。\texttt{SubmissionService}：提交记录管理、历史查询。考试提交前验证考试存在性、PUBLISHED 状态、学生已注册、题目属于考试、提交时间位于考试窗口内，通过后再执行判题、写入提交记录并触发成绩重算。两个接口：\texttt{run}（自测不存记录）和 \texttt{submit}（正式提交存记录）。

\item[exam 模块（6 文件 / 406 行）]
考试全生命周期管理。创建草稿 → 四步向导配置（参数/选题/分值/分配学生）→ 发布 → 学生作答 → 成绩汇总。\texttt{ExamService}：仅 DRAFT 状态可添加题目和学生，发布状态校验（已发布不可再次发布，已归档不可操作）、学生/题目存在性校验、重复添加检测。成绩重算：取每道题最高提交分，按 \texttt{exam\_questions.score} 上限汇总，写入 \texttt{exam\_students.final\_score}。

\item[classes 模块（5 文件 / 154 行）]
班级 CRUD。教师和助教创建班级时自动生成 8 位随机邀请码。学生通过邀请码加入班级，插入 \texttt{student\_class} 记录。教师、助教或管理员调用加入接口返回 403。
...
\begin{itemize}
  ...
  \item \texttt{router/index.ts}：14 条路由，按角色分三级——公开路由（/login）、学生路由（/student/*）、教师工作台路由（/teacher/*）、管理员路由（/admin/*）。教师、助教和管理员可访问教师工作台；助教登录后复用教师端页面。路由守卫在需要时调用 \texttt{/auth/me} 校验后端登录态和角色，学生访问教师工作台会自动重定向到学生仪表盘，管理员路由仅 ADMIN 可访问。未登录或 token 失效访问受保护路由都会重定向 /login。
\end{itemize}

\subsubsection*{组件（8 个）}
...
\texttt{/teacher/dashboard}                  & TeacherDashboard   & 教师/助教 \\
\texttt{/teacher/questions}                  & TeacherQuestions   & 教师/助教 \\
\texttt{/teacher/classes}                    & TeacherClasses     & 教师/助教 \\
\texttt{/teacher/exams/new}                  & ExamWizard         & 教师/助教 \\
\texttt{/teacher/scores}                     & TeacherScores      & 教师/助教 \\
...
\texttt{JudgeService.java} 是系统最核心的文件，关键实现如下：

\textbf{安全检查（validateSql 方法）：} 调用 JSqlParser 解析 SQL，要求提交内容非空、不是多语句、不包含写入或 DDL 关键字，并且根节点必须是单个 \texttt{SELECT} 查询。语法错误返回 BAD REQUEST，非查询类语句返回 FORBIDDEN。

\textbf{临时 Schema 执行（run / executeCase 方法）：} 创建 JDBC 连接 → \texttt{CREATE DATABASE judge\_xxx} → 执行测试用例 \texttt{input\_sql}（CREATE TABLE + INSERT）→ 设置 \texttt{setQueryTimeout} → 执行学生 SQL → 读取 \texttt{ResultSet} → 转为规范化 JSON → 与 \texttt{expected\_output} 比对 → 在 \texttt{finally} 块中 \texttt{DROP DATABASE judge\_xxx}。

\textbf{结果比对（compare 方法）：} 逐一检查列名、行数和单元格值。根据 \texttt{orderSensitive} 决定是否排序后比较。NULL 值特殊处理，数字类型会做规范化后比较。
