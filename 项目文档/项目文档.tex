...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{在线考试}：教师通过四步向导创建考试（配置参数 → 选题 → 设分值 → 分配学生），发布后学生在指定时间窗口内作答。支持多题导航、独立草稿。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item 创建 Schema 和初始化表结构使用管理员连接（需 \texttt{allowMultiQueries}），执行学生 SQL 使用独立低权限连接。
...
系统核心。\texttt{JudgeService}（302 行）：三层安全检查 → 临时 Schema 创建 → 测试用例初始化 → 学生 SQL 执行 → 结果比对 → Schema 清理。管理员连接负责建库（\texttt{allowMultiQueries=true}），学生 SQL 走独立低权限连接（\texttt{allowMultiQueries=false}）。启动时自动清理残留的孤儿 \texttt{judge\_\%} Schema。\texttt{SubmissionService}：提交记录管理、历史查询。考试提交前验证考试存在性、PUBLISHED 状态、学生已注册，通过后触发成绩重算。两个接口：\texttt{run}（自测不存记录）和 \texttt{submit}（正式提交存记录）。

\item[exam 模块（6 文件 / 406 行）]
考试全生命周期管理。创建草稿 → 四步向导配置（参数/选题/分值/分配学生）→ 发布 → 学生作答 → 成绩汇总。\texttt{ExamService}（221 行）：仅 DRAFT 状态可添加题目和学生，发布状态校验（已发布不可再次发布，已归档不可操作）、学生/题目存在性校验、重复添加检测。成绩重算：取每道题最高提交分，按 \texttt{exam\_questions.score} 上限汇总，写入 \texttt{exam\_students.final\_score}。
...
\begin{itemize}
  ...
  \item \texttt{router/index.ts}：14 条路由，按角色分三级——公开路由（/login）、学生路由（/student/*）、教师路由（/teacher/*）、管理员路由（/admin/*）。路由守卫检查 Pinia auth store 的角色字段，学生访问教师路由自动重定向到学生仪表盘，反之同理。未登录访问任何受保护路由都重定向 /login。
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{SqlEditor.vue}（57 行）：SQL 编辑器组件封装，\texttt{v-model} 双向绑定 SQL 代码。
...
\texttt{JudgeService.java}（302 行）是系统最核心的文件，关键实现如下：
...
\textbf{临时 Schema 执行（judge 方法）：} 创建 JDBC 连接 → \texttt{CREATE DATABASE judge\_xxx} → \texttt{USE judge\_xxx} → 执行测试用例 \texttt{input\_sql}（CREATE TABLE + INSERT）→ 设置 \texttt{setQueryTimeout} → 执行学生 SQL → 读取 \texttt{ResultSet} → 转为规范化 JSON → 与 \texttt{expected\_output} 比对 → 在 \texttt{finally} 块中 \texttt{DROP DATABASE judge\_xxx}。
